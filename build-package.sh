...
while (($# >= 1)); do
	case "$1" in
		...
		-i)
			...
			elif [ "$TERMUX_PREFIX" != "/data/data/com.itsaky.androidide/files/usr" ]; then
				termux_error_exit "./build-package.sh: option '-i' is available only when TERMUX_APP_PACKAGE is 'com.itsaky.androidide'"
			else
